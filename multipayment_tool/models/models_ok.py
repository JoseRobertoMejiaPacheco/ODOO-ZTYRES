        ...
        for record in self:
            if record.payment_name == 'out_refund' and record.payment_origin == 'new':
                if record.move_id:
                    ...
                    if record.move_id.l10n_mx_edi_cfdi_uuid:
                        record.send_mail_nc()
    
    def send_mail_nc(self):
        for record in self:
            if record.move_id:
                action_nc = record.move_id.with_context(discard_logo_check=True).action_invoice_sent()
                ctx = action_nc['context']
                invoice_send_wizard_nc = self.env['account.invoice.send'].with_context(
                    ctx,
                    active_ids=record.move_id.ids
        ...
        for record in self:
            if record.payment_name == 'out_refund' and record.payment_origin == 'new' and record.discount_id and not record.move_id:
                ...
                if invoice.move_id.amount_tax > 0:
                    price_unit = round(record.amount_to_apply / 1.16,2)
                else:
