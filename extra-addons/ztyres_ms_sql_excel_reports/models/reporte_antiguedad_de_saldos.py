...
class AntiguedadDeSaldos(models.TransientModel):
    ...
    def get_report(self, fecha):
        ...
        df_pivoted = df2.pivot_table(index=['factura', 'total_de_pagos', 'cobrador', 'fecha_aplicación'], 
                                            columns='tipo', 
                                            values='monto', 
                                            aggfunc='sum', 
        ...
        merged_df = merged_df.drop(columns=['TotalFac', 'restante', 'total_de_pagos'])
        ...
        query3 = """            
            ...
            AND apr.max_date <= %s
            GROUP BY am2."name",am.currency_id
        """
        # Ejecutar la consulta y crear el DataFrame
        self.env.cr.execute(query3, (fecha,))
        ...
        merged_df2['dias de atraso'] = (pd.to_datetime(fecha) - pd.to_datetime(merged_df2['fecha_limite'])).dt.days
        merged_df2['restante'] = merged_df2['total'] - merged_df2['monto']
        
        merged_df2 = merged_df2.drop(merged_df2[merged_df2['restante'] == 0].index)
        merged_df2 = merged_df2.drop(merged_df2[merged_df2['restante'] == ""].index)
        merged_df2 = merged_df2.dropna(subset=['restante'])
        ...
        lista.append(('Antiguedad de saldos', merged_df3))

        #merged_df3.to_excel('/mnt/extra-addons/Reporte Kim Tal Vez.xlsx')
